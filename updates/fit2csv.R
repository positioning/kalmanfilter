...
{ 
  ...
  if (cls=="trackit") 
  {
     ...
     cat(c("pointid","datetime","mptlon","mptlat","varlon","vxy","vyx","varlat",paste(cls,"\n", sep="")), sep=",", file=fln1)
  ...
  else 
  {
   ...
   fln3 <- paste(folder,paste(name,"c.csv",sep = ""), sep = "/")
    if (cls=="kfsst"){
     cat(c("pointid","dataname","year","month","day","mptlon","mptlat","varlon","vxy","vyx","varlat","taglon","taglat","tagsst", "mptsst",paste(cls,"\n", sep="")), sep=",", file=fln1)
     write.table(data.frame(name,fit$date,fit$most.prob.track,fit$var.most.prob.track,fit$nominal.track,fit$SST[,c(1,3)]), append=T, col.names=F, sep=",", na="", file=fln1)
     ...
     write.table(data.frame(fit$theta.active, fit$theta.init, est, std, name),append=T, col.names=F, sep=",", na="", file=fln2)
     cat(c("dataname","fixfirst","fixlast", "varstruct", "nlogL","maxgradcomp", "call",paste(cls,"\n", sep="")), sep=",", file=fln3)
     write.table(data.frame(name,fit[c(15,16,19,2,3)],gsub("\"","'",as.character(fit[20]))),append=T, col.names=F, row.names=F, sep=",", na="", file=fln3)
    }
   if (cls=="kftrack"){
     cat(c("pointid","dataname","year","month","day","mptlon","mptlat","varlon","vxy","vyx","varlat","taglon","taglat",paste(cls,"\n", sep="")), sep=",", file=fln1)
     write.table(data.frame(name,fit$date,fit$most.prob.track,fit$var.most.prob.track,fit$nominal.track), append=T, col.names=F, sep=",", na="", file=fln1)
     # Add labels, rearrange, fill gaps
    ...
    write.table(data.frame(name,fit[c(16,17,20,2,3,21)],gsub("\"","'",as.character(fit[22]))),append=T, col.names=F, row.names=F, sep=",", na="", file=fln3)
    }
  }
}
