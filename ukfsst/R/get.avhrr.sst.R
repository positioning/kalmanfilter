`get.avhrr.sst` <-
function (track, folder = tempdir(), server = "http://coastwatch.pfeg.noaa.gov/coastwatch/CWBrowserWW360.jsp?get=gridData&dataSet=", 
    product = "TAGssta", nday = "8day", centertime = "12") 
{
    ...
    op <- paste("&timePeriod=NDAY&centeredTime=~DATET", centertime, 
        "%3A00%3A00&maxLat=MAXLAT&minLon=MINLON&maxLon=MAXLON&minLat=MINLAT&filetype=.xyz", 
        sep = "")
    for (d in datesteps) {
        ...
        d1 <- (d - ifelse(nday == "3day", 1, 3)) - mdy.date(month = 1, 
            day = 1, year = y1) + 1
        y2 <- date.mdy(d + ifelse(nday == "3day", 1, 4))$year
        d2 <- (d + ifelse(nday == "3day", 1, 4)) - mdy.date(month = 1, 
            day = 1, year = y2) + 1
        filename <- paste(substring(product, 2, 3), y1, fmtDay(d1), 
            "_", y2, fmtDay(d2), "_sst.xyz", sep = "")
