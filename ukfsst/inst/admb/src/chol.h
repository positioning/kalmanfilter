...
dvar_matrix chol(_CONST dvar_matrix& _MM)
{
  ...
  if (MM.colsize()==1)
  {
    ...
    if (MM(mmin,mmin)<=0)
    {
      cout << "Warning: matrix in chol not pd, so diag element added to prevent crash"<<endl;
      MM(mmin,mmin)=1.0e-6;
    }
  ...
  double tmp;
    if (M(1,1)<=0)
    {
      cout << "Warning: matrix in chol not pd, so diag element added to prevent crash"<<endl;
      M(1,1)=1.0e-6; 
      //cerr << "Error matrix not positive definite in chol"
      //  <<endl;
      //ad_exit(1);
    }
  L(1,1)=sqrt(M(1,1));
