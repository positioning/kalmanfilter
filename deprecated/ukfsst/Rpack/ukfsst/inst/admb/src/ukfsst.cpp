...
TRACE(recap_point)
 ...
 TRACE(start_long)
  noOfSstFiles.allocate("noOfSstFiles");
 ...
 for(int i=1; i<=noOfSstFiles; ++i){
  ...
  sstdatemat.allocate(1,noOfSstFiles,1,3,"sstdatemat");
 sstDates.allocate(1,noOfSstFiles);
  sstSolarDate.allocate(1,noOfSstFiles);
 ...
 for (int i = 1; i <= npoint; i++)
 {
  ...
  idx1.allocate(1,npoint);
 idx1.initialize(); 
  idx2.allocate(1,npoint);
 idx2.initialize();
 idx1(1)=1; 
 int j=1; 
 ...
 for (int i = 1; i <= npoint; i++){
  ...
  P.allocate(1,npoint,1,stateDim,1,stateDim);
  PP.allocate(1,npoint,1,stateDim,1,stateDim);
   nphase = 0;
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  vsst.allocate(0.0,50.0,vsst_phase,"vsst");
  bx.allocate(-50.0,50.0,bx_phase,"bx");
  by.allocate(-50.0,50.0,by_phase,"by");
  bsst.allocate(-50.0,50.0,bsst_phase,"bsst");
  a0.allocate(0.0,50.0,a0_phase,"a0");
  b0.allocate(-80.0,80.0,b0_phase,"b0");
  radius.allocate(25,1200,r_phase,"radius");
  nLogL.allocate("nLogL");
  sduu.allocate("sduu");
  sdvv.allocate("sdvv");
  sdD.allocate("sdD");
  sdbx.allocate("sdbx");
  sdby.allocate("sdby");
  sdbsst.allocate("sdbsst");
  sdvx.allocate("sdvx");
  sdvy.allocate("sdvy");
  sdvsst.allocate("sdvsst");
  sdr.allocate("sdr");
  X.allocate(1,npoint,1,stateDim,"X");
  #ifndef NO_AD_INITIALIZE
    X.initialize();
  #endif
  V.allocate(1,npoint,1,stateDim,"V");
  #ifndef NO_AD_INITIALIZE
    V.initialize();
  #endif
  lonlat0.allocate(1,stateDim,"lonlat0");
  #ifndef NO_AD_INITIALIZE
    lonlat0.initialize();
  #endif
 int gotlon=0; 
...
dvariable model_parameters::wave(_CONST dvar_vector& w, _CONST dvector& p)
{
  dvector cw=value(w);
  double T=cw*p; 
  double B=sum(cw); 
...
dvariable model_parameters::weight(dvariable& distsq, dvariable& radsq)
{
  return pow(1.0-pow(distsq/radsq,1.5),3.0); 
}

dvariable model_parameters::getSST(const dvariable& time, dvariable lon, dvariable lat, dvariable rad)
{
...
dvar_vector model_parameters::z(const dvar_vector& alpha)
{
  dvariable t1, t2;
  dvar_vector ret(1,2);
...
dvar_vector model_parameters::zInv(const dvar_vector& y)
{
  dvar_vector ret(1,2);
...
dvar_vector model_parameters::F(const dvar_vector& x, const double& dt)
{
  int n1=x.indexmin();
  int n2=x.indexmax();
...
dvar_matrix model_parameters::Q(const double& dt)
{
  int stateDim=2;
...
dvar_vector model_parameters::G(const dvar_vector& x, const dmatrix& data)
{
  int maxObsDim=3; 
  int r1=data.rowmin(); 
  int r2=data.rowmax(); 
...
dvar_vector model_parameters::Glast(const dvar_vector& x, const dmatrix& data)
{
  int maxObsDim=3; 
  int r1=data.rowmin(); 
  int r2=data.rowmax(); 
...
dvar_matrix model_parameters::R(const dmatrix& data)
{
  int maxObsDim=3;
  int n1=data.rowmin();
  int n2=data.rowmax();
...
dvariable model_parameters::unscentedKalmanFilter(const dvar_vector& x0, const dvar_matrix& varx0, const dmatrix& data, const ivector&  idx1, const ivector& idx2, const dvector& times, dvar_matrix& X, dvar_matrix& V, d3_array& Pall, d3_array& PPall, const double& kappa)
{
