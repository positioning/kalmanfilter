...
get.sst.from.server <- function (track, lonlow, lonhigh, latlow, lathigh, folder = tempdir(), 
    removeland = TRUE, minus180 = FALSE, offsetx = 0, offset.cutoff = 360, ftpurl = "downloads.psl.noaa.gov") 
{
    require(date)
    require(ncdf4)
    ...
    lathigh <- ifelse(lathigh > 89.5, 89.5, trunc(lathigh) + 
        0.5)
    lonlow <- ifelse(lonlow < 0, trunc(lonlow) + 360 - 0.5, trunc(lonlow) - 
        0.5)
    lonhigh <- ifelse(lonhigh < 0, trunc(lonhigh) + 360 + 0.5, 
        trunc(lonhigh) + 0.5)
    ...
    for (i in 1:nt) {
        ...
        for (j in 1:length(lon)) {
            m180 <- ifelse(lon[j] > 180, lon[j] - 360, lon[j]) + 
                offsetx
